...
function Mdiag(const d: array of double): TDMatrix;
...
begin
  ...
  rows := NewRows;
  cols := NewCols;
  //fillbyte(data, sizeof(data), 0);  // data may be garbage that just looks like an existing array -> bad fix?
  Setlength(data, rows * cols);
  // Setlength not always fills the array with zeros
