...
type
  ...
  TFMain = class(TForm)
    ...
    BTest8: TButton;
    BTest9: TButton;
    procedure BTest10Click(Sender: TObject);
    procedure BTest11Click(Sender: TObject);
...
procedure TFMain.BTest10Click(Sender: TObject);
var A, B, C: TDMatrix;
begin
  A := Meye(3);
  A := Mones(2, 2);
...
procedure TFMain.BTest11Click(Sender: TObject);
var A, V, U, S: TDMatrix;
    r, c: integer;
begin
  c := 1 + Random(3);
  r := c + Random(5);
  ...
  S := SVD(A, U, V);
  MAddToStringListWL('A', A, Memo.lines);
  MAddToStringListWL('V', V, Memo.lines);
  MAddToStringListWL('U', U, Memo.lines);
  MAddToStringListWL('S', S, Memo.lines);
  //MAddToStringListWL('A - U*S*V''', A - U * S * V.t, Memo.lines);
   Memo.lines.Add(format('Err = %.6g',[MmaxAbs(A - U * S * V.t)]));
end;
